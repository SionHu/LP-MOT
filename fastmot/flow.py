...
from networks.LiteFlowNet3 import LiteFlowNet3, normalize_image, square_image
...
class FarnebackFlow(Flow):
    def __init__(self, size, cuda=True, **kwargs):
        pass

    def init(self, frame):
        self.prev = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
        # frame = normalize_image(frame).unsqueeze(0)
        #
        # self.intWidth = frame.shape[3]
        # self.intHeight = frame.shape[2]
        #
        # self.intPreprocessedWidth = int(math.floor(math.ceil(self.intWidth / 32.0) * 32.0))
        # self.intPreprocessedHeight = int(math.floor(math.ceil(self.intHeight / 32.0) * 32.0))
        #
        # self.prev = square_image(frame, self.intHeight, self.intWidth, self.intPreprocessedHeight, self.intPreprocessedWidth, self.cuda)
        # # print(self.prev.shape)
        # # self.prev = frame

    def predict(self, frame, tracks):
        TINY_FLOWNET_SCALE = 1

        frame = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
        flow = cv2.calcOpticalFlowFarneback(self.prev, frame, None, pyr_scale = 0.5, levels = 5, winsize = 11, iterations = 5, poly_n = 5, poly_sigma = 1.1, flags = 0)
        # frame = normalize_image(frame).unsqueeze(0)
        # frame = square_image(frame, self.intHeight, self.intWidth, self.intPreprocessedHeight, self.intPreprocessedWidth, self.cuda)
        # # print(frame.shape)
        flow = self.flownet(self.prev, frame)

        next_bboxes = {}
        for track in tracks:
            old_box = track.tlbr
            # TODO: mutate width and height of old_box into new_box
            center = get_center(old_box)
            # delta = flow[i, center[0] // TINY_FLOWNET_SCALE, center[1] // TINY_FLOWNET_SCALE, :]
            delta = flow[int(center[0]) // TINY_FLOWNET_SCALE, int(center[1]) // TINY_FLOWNET_SCALE, :]
            new_box = np.array([
                old_box[0] + delta[0],
                old_box[1] + delta[1],
                old_box[2] + delta[0],
            ...
            next_bboxes[track.trk_id] = np.rint(new_box)
        print(next_bboxes)
        self.prev = frame

        # TODO: anything that is not None indicates that the optical flow was successful. None means that it failed
        homography = np.eye(3)
        return next_bboxes, homography


class LPMOTFlow(Flow):
    def __init__(self, size, cuda=False, **kwargs):
        if cuda:
            self.flownet = LiteFlowNet3().eval().cuda()
        else:
            self.flownet = LiteFlowNet3().eval()
        self.cuda = cuda
    ...
    def predict(self, frame, tracks):
        ...
        # print(frame.shape)
        # flow = self.flownet(self.prev, frame)
        # _ = self.flownet(frame, frame).cpu().detach().numpy()
        flow = cv2.calcOpticalFlowFarneback(self.prev, frame_, None, pyr_scale = 0.5, levels = 5, winsize = 11, iterations = 5, poly_n = 5, poly_sigma = 1.1, flags = 0)
