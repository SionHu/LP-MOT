...
def draw_tracks(frame, tracks, show_flow=False, show_cov=False):
    for track in tracks:
        draw_bbox(frame, track.tlbr, get_color(track.trk_id), 2, track.dist, track.gps, str(track.trk_id))
        if show_flow:
...
def draw_detections(frame, detections):
    for det in detections:
        print("in draw_detections:", det.dist)
        draw_bbox(frame, det.tlbr, (255, 255, 255), 1, det.dist, det.gps)


def draw_flow_bboxes(frame, tracker):
    for tlbr in tracker.flow_bboxes.values():
        draw_bbox(frame, tlbr, 0, 1, 1, 1)
...
def draw_bbox(frame, tlbr, color, thickness, distance, gps, text=None):
    ...
    if text is not None:
        ...
        cv2.putText(frame, text, (tl[0], tl[1] + text_height - 1), cv2.FONT_HERSHEY_DUPLEX,
                    0.5, 0, 1, cv2.LINE_AA)
        # distance
        cv2.putText(frame, f'{distance:.2f}', (tl[0], tl[1]-2), cv2.FONT_HERSHEY_SIMPLEX,
                    0.5, 0, 1, cv2.LINE_AA)
        # GPS
        cv2.putText(frame, f'{gps[0]:.6f}, {gps[1]:.6f}', (br[0], br[1]+1), cv2.FONT_HERSHEY_SIMPLEX,
                    0.5, 0, 2, cv2.LINE_AA)
