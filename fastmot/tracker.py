...
# from .flow import KLTFlow
from .flow import LPMOTFlow as KLTFlow
...
class MultiTracker(Tracker):
    def __init__(self, size, metric,
        ...
        flow_cfg : SimpleNamespace, optional
            Flow configuration.
        """
        super().__init__(size, metric, max_age, age_penalty, motion_weight,
            max_assoc_cost, max_reid_cost, iou_thresh, duplicate_thresh,
            occlusion_thresh, conf_thresh, confirm_hits, history_size)
        ...
        self.flow = KLTFlow(self.size, **vars(flow_cfg))
        # self.frame_rect = to_tlbr((0, 0, *self.size))
        self.frame_rect = to_tlbr((0.0, 0.0, self.size[0], self.size[1]))
